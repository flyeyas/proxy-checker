...
- 检测日志会记录手动检测和自动任务的开始时间、结束时间、模式、轮次、并发、数量和结果摘要。
- 结果按有效、失效、我的仓库分组展示。
...
- 支持每隔 N 小时执行，或按所选计划时区每天固定 HH:MM 执行。
- 支持选择计划时区；当前时间、固定执行时间和下次执行时间会按同一个时区展示。
- 支持只检测新代理，或强制检测全部代理。
...
- 检测轮次、并发、请求超时、协议识别超时、登录有效期、日志保留条数、默认时区都可以在页面设置里修改。
- 检测轮次默认 2 轮，上限保持 3 轮，避免免费代理批量检测时被过度放慢。
- 登录密码可以在自托管 Python 服务里通过设置弹窗修改；如果部署用环境变量强制指定密码，页面只能临时生效，重启后仍以环境变量为准。
...
- 新增检测日志弹窗，手动检测和自动任务都会记录开始/结束时间和结果摘要。
- 新增全局设置弹窗，统一管理检测轮次、并发、超时、登录密码、日志保留和默认时区。
- 自动模式新增计划时区，修复“服务器时间”和“下次执行”混用浏览器本地时区导致的显示错位。
- 检测轮次上限恢复为 3 轮。
