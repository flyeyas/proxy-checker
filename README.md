...
本地 `docker compose up -d --build` 会生成类似 `proxy-checker:v0.1.0` 的镜像。
...
- 推送 `v*` tag：构建并推送 tag 标签，例如 `v0.1.0`。
...
```text
ghcr.io/<你的 GitHub 用户名或组织名>/<仓库名>:v0.1.0
```

例如：

```bash
docker pull ghcr.io/flyeyas/proxy-checker:v0.1.0
```
...
docker pull ghcr.io/YOUR_GITHUB_USERNAME/proxy-checker:v0.1.0
```
...
```text
IMAGE=ghcr.io/YOUR_GITHUB_USERNAME/proxy-checker:v0.1.0
