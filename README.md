...
- 支持只检测新代理，或强制检测全部代理。
- 支持稳定代理、包含不稳定、只入库等级 A、只入库等级 B、只入库等级 AB、全部结果留档等入库策略。
...
自托管 Python 服务内置一个最小可用代理网关，默认监听 `127.0.0.1:7890`，从“我的仓库”里自动选择等级 A / B 的 HTTP(S) 代理进行转发。它支持普通 HTTP 代理请求和 HTTPS `CONNECT` 隧道，因此可以像本地代理一样调用：
...
- 默认只使用仓库里等级 A / B 的代理。
- 默认扫描当前部署里的全部仓库 token；如果只想使用某个浏览器仓库，设置 `PROXY_GATEWAY_TOKEN`。
- 只使用 HTTP / HTTPS 上游代理；仓库中的 SOCKS4 / SOCKS5 上游代理会暂时跳过。
- Docker Compose 默认只绑定宿主机 `127.0.0.1:7890`，避免 VPS 变成公网开放代理。

如果明确要让其它机器访问这个代理端口，需要自行承担安全风险，并在 `.env` 里改成：

```text
PROXY_HOST_BIND=0.0.0.0
PROXY_HOST_PORT=7890
```
...
- 自动任务入库策略新增只入库等级 A、只入库等级 B、只入库等级 AB。
- 后端入库策略校验统一为白名单，自动任务保存和仓库合并逻辑保持一致。
- 新增最小可用统一代理转发端口，默认使用仓库等级 A / B 的 HTTP(S) 代理。
- Docker 镜像标签更新为 `v0.1.1`。
...
- 自动任务支持多种入库策略：只入库稳定代理、包含不稳定代理、只入库等级 A、只入库等级 B、只入库等级 AB、全部结果留档。
...
- 推送 `v*` tag：构建并推送 tag 标签，例如 `v0.1.1`。
...
```text
ghcr.io/<你的 GitHub 用户名或组织名>/<仓库名>:v0.1.1
```
...
| `log_limit` | `LOG_LIMIT` | `100` | 每个 token 保留的检测日志条数 |
| `timezone` | `APP_TIMEZONE` | `UTC` | 默认展示和计划任务时区 |
| `port` | `PORT` | `8888` | HTTP 服务端口 |
| `proxy_gateway_enabled` | `PROXY_GATEWAY_ENABLED` | `true` | 是否启用统一代理转发端口 |
| `proxy_gateway_bind` | `PROXY_GATEWAY_BIND` | `127.0.0.1` | 代理网关监听地址；Docker 内由 Compose 设置为 `0.0.0.0` |
| `proxy_gateway_port` | `PROXY_GATEWAY_PORT` | `7890` | 代理网关容器内监听端口 |
