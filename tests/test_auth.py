...
from proxy_checker.services.auth_service import AuthService, create_runtime_auth_service


class AuthServiceTest(unittest.TestCase):
    def test_password_and_token(self):
        auth = AuthService("secret", "signing", 60, "proxy_checker_auth")
    ...
    def test_disabled_auth_allows_request(self):
        auth = AuthService("", "signing", 60, "proxy_checker_auth")
    ...
    def test_headers_and_cookie_token(self):
        ...
        self.assertEqual(auth.cookie_token("proxy_checker_auth=cookie-token"), "cookie-token")

    def test_expired_token_rejected(self):
        auth = AuthService("secret", "signing", 1, "proxy_checker_auth")
